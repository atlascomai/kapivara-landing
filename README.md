...
O QR Code do rodape aponta direto para o WhatsApp `+55 11 93623-6342`.

A pagina `qr.html` segue como fallback para links antigos, dispara o evento `qr_code_open` no `dataLayer` e tenta enviar o payload para `POST /api/qr-open`. Para contabilizar leituras e incluir o total no e-mail de resumo diario, esse endpoint precisa ser implementado em um backend seguro ou ferramenta de analytics conectada ao e-mail diario.
